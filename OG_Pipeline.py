...
class AllShotsDialog(QDialog):
    ...
    def __init__(self, shots_parent, parent=None, stage_subpath="", stages=None):
        ...
        print("[OG_Pipeline] AllShots: shots_parent=", shots_parent,
              "subpath=", repr(self._stage_subpath), "has_subpath=", has_subpath,
              "stages_cfg=", bool(self._stages))
        _dbg = 0
        for d in names:
            full = os.path.join(shots_parent, d)
            if not os.path.isdir(full):
                continue
            if has_subpath and _dbg < 3:
                _dbg += 1
                ex = expand_stage_bases(full, self._stage_subpath)
                print("[OG_Pipeline]   shot=", d, "matched=",
                      [os.path.relpath(x, full) for x in ex])
            if has_subpath:
